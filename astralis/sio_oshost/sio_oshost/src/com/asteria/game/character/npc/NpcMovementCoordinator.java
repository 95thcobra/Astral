...
public final class NpcMovementCoordinator {
    ...
    public void sequence() {
        ...
        if (random.inclusive(13) == 5 && npc.getMovementQueue().isMovementDone()) {
            switch (state) {
            case HOME:
            	Position randomWalkTo = npc.getPosition().copy().random(radius);
            	ClippedPathFinder.getPathFinder().findRoute(npc, randomWalkTo.getX(), randomWalkTo.getY(), false, 0, 0);
                state = State.AWAY;
                break;
            case AWAY:
            	ClippedPathFinder.getPathFinder().findRoute(npc, npc.getOriginalPosition().getX(), npc.getOriginalPosition().getY(), false, 0, 0);
                state = State.HOME;
                break;
            }
        }
