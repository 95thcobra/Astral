...
import com.asteria.pf.Path;
import com.asteria.pf.PathFinder;
import com.asteria.pf.StraightPathFinder;
import com.asteria.task.Task;
import com.asteria.utility.RandomGen;
import com.asteria.utility.clip.ClippedPathFinder;
...
public final class MovementQueue {
    ...
    public void walk(Position position) {
        ...
        Path p = pf.findPath(character, position, true);
        reset();
        while (!p.getPoints().isEmpty()) {
            com.asteria.pf.Point point = p.getPoints().poll();
            addToPath(new Position(point.getX(), point.getY(), position.getZ()));
        }
        finish();
    }
    ...
    private static final class CharacterFollowTask extends Task {
        ...
        public void execute() {
            ...
            if(character.getType() == NodeType.NPC) {
            	...
            	ClippedPathFinder.getPathFinder().findRoute(character, leader.getPosition().getX(), leader.getPosition().getY(), true, 0, 0);
            }
            //character.getMovementQueue().walk(leader.getPosition().copy());
        }
