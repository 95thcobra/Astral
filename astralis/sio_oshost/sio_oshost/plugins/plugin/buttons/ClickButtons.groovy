    ...
    void execute(Player player, ButtonClickPlugin context) {
        ...
        if (player.rights.DEVELOPER || player.rights.ADMINISTRATOR)
            switch (button) {
                ...
                case 4026: // Running Toggle (orb)
                case 74214: // Running Toggle (interface)
                    if (player.movementQueue.running == false) {
                        player.movementQueue.running = true
                        player.messages.sendByteState(173, 1)
                        return
                    }
                    if (player.movementQueue.running == true) {
            ...
            //* ALL NEW MAGIC TELES
                ...
                case 108003:
                    player.messages.sendCloseWindows()
                    player.messages.sendInterface(42000)
                    break
                case 3082:
                    player.messages.sendCloseWindows()
                    player.messages.sendInterface(54000)
                    break
                case 7053:
                ...
                case 30108:
                    ...
                    break
                default:
                    player.messages.sendMessage("Unhandled Button : " + button)
                    break
            }
    }
}